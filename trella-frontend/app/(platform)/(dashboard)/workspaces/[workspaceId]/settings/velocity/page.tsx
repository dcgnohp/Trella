...
export default function VelocitySettingsPage() {
  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 700 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, color: "#172B4D", margin: "0 0 8px" }}>
        Velocity Configuration
      </h1>
      <p style={{ fontSize: 14, color: "#5E6C84", margin: "0 0 32px" }}>
        Configure how story points translate to time estimates. Used for auto-calculating due dates.
      </p>

      <div style={{ background: "#FFFFFF", border: "1px solid #DFE1E6", borderRadius: 8, padding: "24px", marginBottom: 24 }}>
        <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#172B4D", marginBottom: 6 }}>
          Hours per story point
        </label>
        <p style={{ fontSize: 12, color: "#5E6C84", margin: "0 0 12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <input
            ...
            style={{ width: 100, padding: "6px 10px", fontSize: 14, border: "1px solid #DFE1E6", borderRadius: 4, outline: "none", color: "#172B4D" }}
          />
          <span style={{ fontSize: 13, color: "#5E6C84" }}>hours / point</span>
          <button
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid #DFE1E6", borderRadius: 8, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #DFE1E6", background: "#F4F5F7" }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: "#172B4D" }}>Story point reference</span>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#F4F5F7" }}>
              {["Points", "Hours", "Days"].map(col => (
                <th key={col} style={{ padding: "8px 20px", fontSize: 11, fontWeight: 700, color: "#5E6C84", textTransform: "uppercase", textAlign: "left", borderBottom: "1px solid #DFE1E6" }}>{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {FIBONACCI.map(pts => {
              ...
              return (
                <tr key={pts} style={{ borderBottom: "1px solid #F4F5F7" }}>
                  <td style={{ padding: "10px 20px", fontSize: 13, color: "#172B4D", fontWeight: 600 }}>{pts}</td>
                  <td style={{ padding: "10px 20px", fontSize: 13, color: "#172B4D" }}>{hours.toFixed(1)}h</td>
                  <td style={{ padding: "10px 20px", fontSize: 13, color: "#5E6C84" }}>≈ {days.toFixed(1)} days</td>
                </tr>
              );
            })}
          </tbody>
