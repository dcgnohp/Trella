...
class WorkflowsRepository:
    ...
    def list_by_workspace(self, session: Session, workspace_id: uuid.UUID) -> list[Workflow]:
        statement = select(Workflow).where(Workflow.workspace_id == workspace_id).order_by(col(Workflow.created_at))
...
class WorkflowTransitionsRepository:
    def create(self, session: Session, transition: WorkflowTransition) -> WorkflowTransition:
        session.add(transition)
        session.flush()
        return transition

    def get(self, session: Session, transition_id: uuid.UUID) -> WorkflowTransition | None:
        return session.get(WorkflowTransition, transition_id)

    def list_by_workflow(self, session: Session, workflow_id: uuid.UUID) -> list[WorkflowTransition]:
        statement = select(WorkflowTransition).where(WorkflowTransition.workflow_id == workflow_id)
        return list(session.exec(statement).all())
