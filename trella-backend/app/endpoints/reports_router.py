...
def get_sprint_velocity(
    ...
    done_cols = set(
        session.exec(
            ...
            .where(
                Project.workspace_id == workspace_id, BoardColumn.status_key == "DONE"
            )
        ).all()
    )
    ...
    for sprint in sprints:
        tasks = list(
            session.exec(select(Task).where(Task.sprint_id == sprint.id)).all()
        )
...
def get_sprint_burndown(
    ...
    in_progress_cols = set(
        session.exec(
            ...
            .where(
                ...
                BoardColumn.status_key == "IN_PROGRESS",
            )
        ).all()
    )
    ...
    todo = [
        t
        for t in tasks
        if t.column_id not in done_cols and t.column_id not in in_progress_cols
    ]
