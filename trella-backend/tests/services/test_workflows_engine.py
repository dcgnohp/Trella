...
def _setup_workspace_scrum(session: Session) -> tuple[Workspace, Project, User, Task]:
    ...
    user = User(email="pm@example.com", full_name="Project Manager", hashed_password="pw")
    session.add(user)
    session.commit()
    session.refresh(user)

    project = Project(workspace_id=workspace.id, name="Eng Project", key="ENG", created_by=user.id)
    session.add(project)
    session.commit()
    session.refresh(project)

    # Add project member
    pm = ProjectMember(project_id=project.id, user_id=user.id, project_role=ProjectRole.PROJECT_ADMIN.value, status=MemberStatus.ACTIVE.value)
    session.add(pm)
    session.commit()
    ...
    # Columns
    col_todo = BoardColumn(board_id=board.id, name="Backlog", status_key="todo", position=0)
    col_done = BoardColumn(board_id=board.id, name="Done", status_key="done", position=1)
...
def test_kanban_mode_bypasses_workflow_validation(session: Session):
    ...
    user = User(email="trello@example.com", full_name="Trello User", hashed_password="pw")
    session.add(user)
    session.commit()
    
    project = Project(workspace_id=workspace.id, name="Trello Project", key="TR", created_by=user.id)
    session.add(project)
    session.commit()
    
    pm = ProjectMember(project_id=project.id, user_id=user.id, project_role=ProjectRole.PROJECT_MEMBER.value, status=MemberStatus.ACTIVE.value)
    session.add(pm)
    session.commit()
    ...
    cs_b = CustomStatus(workspace_id=workspace.id, name="Col B", canonical_status="DONE")
    session.add(cs_b)
    session.commit()
    
    task = Task(project_id=project.id, board_id=board.id, column_id=col_a.id, title="Trello Card", position=0)
    session.add(task)
    session.commit()

    service = TasksService(activity_logs_service=_DummyActivityLogs(), notification_service=_DummyNotifications())
    
    # KANBAN mode allows moving from Col A -> Col B freely without workflow constraints
    updated = service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_b.id), user)
...
def test_scrum_mode_enforces_workflow_transitions(session: Session):
    ...
    # Find the statuses
    cs_backlog = session.exec(select(CustomStatus).where(CustomStatus.workspace_id == workspace.id, CustomStatus.name == "Backlog")).first()
    cs_selected = session.exec(select(CustomStatus).where(CustomStatus.workspace_id == workspace.id, CustomStatus.name == "Selected for Development")).first()
    cs_done = session.exec(select(CustomStatus).where(CustomStatus.workspace_id == workspace.id, CustomStatus.name == "Done")).first()
    ...
    with pytest.raises(HTTPException) as exc:
        service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_done.id), user)
    assert exc.value.status_code == 400
    assert "transition comment is required" in exc.value.detail

    # 2. Test valid transition with required validator
    updated = service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_done.id, transition_comment="Finished task!"), user)
    ...
    task = service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_selected.id), user)
    ...
    cs_inprogress = session.exec(select(CustomStatus).where(CustomStatus.workspace_id == workspace.id, CustomStatus.name == "In Progress")).first()
    task = service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_inprogress.id), user)
    ...
    outsider = User(email="outsider@example.com", full_name="Outsider", hashed_password="pw")
    session.add(outsider)
    session.commit()
    
    # Add outsider as project member so they pass project authorization check
    session.add(ProjectMember(project_id=project.id, user_id=outsider.id, project_role=ProjectRole.PROJECT_MEMBER.value, status=MemberStatus.ACTIVE.value))
    session.commit()

    cs_codereview = session.exec(select(CustomStatus).where(CustomStatus.workspace_id == workspace.id, CustomStatus.name == "Code Review")).first()
    
    with pytest.raises(HTTPException) as exc:
        service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_codereview.id), outsider)
    assert exc.value.status_code == 403
    assert "Only the current assignee can transition" in exc.value.detail

    # Transition by assignee should succeed
    task = service.update_task(session, task.id, TaskUpdate(custom_status_id=cs_codereview.id), user)
